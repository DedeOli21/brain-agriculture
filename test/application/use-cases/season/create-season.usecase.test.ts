...
describe('CreateSeasonUseCase', () => {
  ...
  it('should call findFarmById on farmRepository with correct farm ID', async () => {
    const data: CreateSeasonDto = { name: 'Spring', year: '2023', farmId: '123' };

    jest.spyOn(farmRepository, 'findFarmById').mockResolvedValue({ id: '123', name: 'Test Farm' } as Farm);
  ...
  it('should throw NotFoundException if farm is not found', async () => {
    ...
    await expect(createSeasonUseCase.execute(data)).rejects.toThrow(NotFoundException);
  });

  it('should call create on seasonRepository with correct data', async () => {
    const data: CreateSeasonDto = { name: 'Spring', year: '2023', farmId: '123' };
    const farm: Farm = { id: '123', name: 'Test Farm' } as Farm;
    const season: Season = { id: '456', name: 'Spring', year: '2023', farm, crops: [] } as Season;
