...
describe('FindByDocumentUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    findByDocumentUseCase = module.get<FindByDocumentUseCase>(
      FindByDocumentUseCase,
    );
  ...
  it('should throw BadRequestException if document is invalid', async () => {
    ...
    await expect(
      findByDocumentUseCase.execute('invalid-document'),
    ).rejects.toThrow(
  ...
  it('should call findProducerByDocument with correct document', async () => {
    ...
    (producerRepository.findProducerByDocument as jest.Mock).mockResolvedValue(
      producer,
    );

    const result = await findByDocumentUseCase.execute(document);

    expect(producerRepository.findProducerByDocument).toHaveBeenCalledWith(
      document,
    );
